defmodule Aoc.Y2021.Day11 do
  ...
  def simulate(_, 0, count), do: count
  ...
  def find_synchronized_flash(grid, steps \\ 1) do
    grid = grid |> increase_one() |> increase_adjacent([])
  ...
  def update_adjacent(grid, {i, j}) do
  ...
  def count_flash(grid) do
    Enum.count(Map.values(grid), fn v -> v > 9 end)
  end

  def set_zero(grid) do
